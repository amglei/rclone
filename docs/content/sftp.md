...
Here are the possible system metadata items for the sftp backend.

| Name | Help | Type | Example | Read Only |
|------|------|------|---------|-----------|
| atime | Time of last access | RFC 3339 | 2006-01-02T15:04:05.999999999Z07:00 | Y |
| gid | Group ID of owner | decimal number | 1000 | Y |
| mode | File type and mode | octal, unix style | 100644 | Y |
| mtime | Time of last modification | RFC 3339 | 2006-01-02T15:04:05.999999999Z07:00 | Y |
| uid | User ID of owner | decimal number | 1000 | Y |

The metadata is read from the SFTP stat info so it works against any
standard SFTP server without server-side changes. The SFTP protocol
(version 3) reports times with one-second precision only and has no way
of carrying the `ctime` or `btime` metadata, so those keys are not
